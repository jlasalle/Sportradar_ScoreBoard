...
public class GlobalDemoTest {
	...
	public void globalTest_A() {
		...
		// Game insertion		
		Game mexCanGame = ctrl.startGame(MEX, CAN);
		Game spaBraGame = ctrl.startGame(SPA, BRA);
		Game gerFraGame = ctrl.startGame(GER, FRA);
		Game uruItaGame = ctrl.startGame(URU, ITA);
		Game argAusGame = ctrl.startGame(ARG, AUS);

		//Score update
		scoreCtrl.updateScore(mexCanGame, MEX_SCORE, CAN_SCORE);
		scoreCtrl.updateScore(spaBraGame, SPA_SCORE, BRA_SCORE);
		scoreCtrl.updateScore(gerFraGame, GER_SCORE, FRA_SCORE);
		scoreCtrl.updateScore(uruItaGame, URU_SCORE, ITA_SCORE);
		scoreCtrl.updateScore(argAusGame, ARG_SCORE, AUS_SCORE);
		...
		assertEquals(result, dis.genSummary());
	}
